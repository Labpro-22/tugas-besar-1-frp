Pastika seluruh output harus melalui view. Maksudnya adalah layer GUI hanya untuk tampilan. Jadi untuk semua perubahan game state itu tetap diwajibkan di logic C++ nya, bukan di layer GUI. Dikhawatirkan ada yang menggunakan game engine atau logic yang tercampur di layer GUI. Layer GUI diperbolehkan untuk input/output, tidak harus input dari CLI. Format atau semua hal yang harus di keluarkan (output) adalah sebagai berikut:
